...
export default function AddProductScreen() {
  ...
  const scanProductFromImage = async (uri: string) => {
    ...
    try {
      ...
      const nextDesc = nextDescCandidate && nextDescCandidate.trim().length >= 20 ? nextDescCandidate : nextDescFallback;
          ...
          <View style={styles.formGroup}>
              ...
              <View style={{ marginTop: sizes.s, flexDirection: "row", justifyContent: "space-between", gap: sizes.s }}>
                ...
                >
                  <Text style={styles.draftButtonText}>
                    {lastScannedUri === imageUri ? "Scanned" : "Scan"}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    const inferredCategory = inferCategoryValue(options, scanResult?.labels, scanResult?.raw_text, objectType);
            ...
            {scanResult?.candidates ? (
              ...
              >
                ...
                <Text style={{ color: colors.text, opacity: 0.8 }}>
                  Price: {normalizePrice(scanResult.candidates.price) || "(not found)"} {scanResult.candidates.currency || ""}
                </Text>
