...
async def predict_risk(request: RiskPredictionRequest) -> RiskPredictionResponse:
    ...
    try:
        ...
        features = np.array([[request.age, mean_bp, request.bs, request.heart_rate]], dtype=np.float64)
        ...
        return RiskPredictionResponse(
            is_high_risk=is_high_risk, risk_probability=risk_probability, message=message, mean_bp=mean_bp
        )

    except Exception as e:
        logger.error(f"Risk prediction error: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Risk prediction failed: {str(e)}"
        )
