...
export default function ProductCategories() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productsAPI.getCategories().then((res) => res.data as ProductCategory[]),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (label: string) => productsAPI.createCategory(label.trim()),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, label }: { id: number; label: string }) => productsAPI.updateCategory(id, label.trim()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['product-categories'] });
      handleCloseModal();
    },
