...
export default function RecipeCategories() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => recipesAPI.getCategories().then((res) => res.data as RecipeCategory[]),
  });
  ...
  const createMutation = useMutation({
    mutationFn: (label: string) => recipesAPI.createCategory(label.trim()),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, label }: { id: number; label: string }) => recipesAPI.updateCategory(id, label.trim()),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['recipe-categories'] });
      handleCloseModal();
    },
